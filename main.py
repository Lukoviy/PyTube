...
def get_message(message):
        bot.send_message(message.from_user.id, "Пришлите ссылку на Youtube видео, которое хотите скачать")

@bot.message_handler(content_types = ["text"])
def get_message_youtube(message):
    if message.text.startswith("https://www.youtube.com/"):
        ...
        bot_msg_res = bot.send_message(message.from_user.id, "Выберите качество видео", reply_markup=markup)
        # bot.delete_message(chat_id=message.from_user.id, message_id=message.id)
    elif message.text == ("360p"):
        download_video(message, "360p", url, bot_msg_res, msg_url)
    elif message.text == ("480p"):
        download_video(message, "480p", url, bot_msg_res, msg_url)
    elif message.text == ("720p"):
        download_video(message, "720p", url, bot_msg_res, msg_url)
    elif message.text == ("1080p"):
        download_video(message, "1080p", url, bot_msg_res, msg_url)
    elif message.text == ("Аудио"):
        download_audio(message, url, bot_msg_res, msg_url)
    else:
        bot.send_message(message.from_user.id, "Неверная ссылка или видео невозможно скачать")
